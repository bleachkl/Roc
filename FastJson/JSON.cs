    ...
    internal class deserializer
    {
        ...
        private object ChangeType(object value, Type conversionType)
        {
            ...
            else if (conversionType == typeof(long))
            {
                ...
                return (string)value;
            //Edit
            else if (conversionType?.IsEnum == true)
        ...
        internal static Dictionary<Type, List<JsonKeyAttribute>> ms_JsonKeyAttributes = new Dictionary<Type, List<JsonKeyAttribute>>();
        /// <summary>
        /// 获取被Jsonkey标记的属性或字段取得反序列化对应字段
        /// </summary>
        /// <param name="type"></param>
        /// <returns></returns>
        public static List<JsonKeyAttribute> GetJsonKeyAttributes(Type type)
        {
            if (type == default)
                return default;
            if (ms_JsonKeyAttributes.ContainsKey(type))
            {
                return ms_JsonKeyAttributes[type];
            }
            else
            {
                var jsonkeyAtts = new List<JsonKeyAttribute>();
                var properties = type.GetProperties();
                var fields = type.GetFields();
                if (properties?.Length > 0)
                    foreach (var property in properties)
                        if (property.GetCustomAttribute<JsonKeyAttribute>() is JsonKeyAttribute jsonKeyAttribute
                            && !string.IsNullOrWhiteSpace(jsonKeyAttribute.JsonKey))
                        {
                            jsonKeyAttribute.m_VariableName = property.Name;
                            jsonkeyAtts.Add(jsonKeyAttribute);
                        }
                if (fields?.Length > 0)
                    foreach (var field in fields)
                        if (field.GetCustomAttribute<JsonKeyAttribute>() is JsonKeyAttribute jsonKeyAttribute
                            && !string.IsNullOrWhiteSpace(jsonKeyAttribute.JsonKey))
                        {
                            jsonKeyAttribute.m_VariableName = field.Name;
                            jsonkeyAtts.Add(jsonKeyAttribute);
                        }
                ms_JsonKeyAttributes.Add(type, jsonkeyAtts);
                return jsonkeyAtts;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        internal static bool RegardAs(Type instanceType, Type targetType)
        {
        ...
        /// <param name="instance"></param>
        /// <returns></returns>
        internal static bool RegardAs<T>(Type instanceType)
        {
        ...
        internal object ParseDictionary(Dictionary<string, object> d, Dictionary<string, object> globaltypes, Type type, object input)
        {
            ...
            if (o == null)
            {
                ...
                if (!targetType.IsInterface
                    ...
                    && RegardAs(type, targetType)
                    && targetType.IsSubclassOf(type))
                    creatType = targetType;
                if (_params.ParametricConstructorOverride)
                    o = System.Runtime.Serialization.FormatterServices.GetUninitializedObject(creatType);
                else
                    o = Reflection.Instance.FastCreateInstance(creatType);
            }
            int circount = 0;
            ...
            var jsonKeyAtts = GetJsonKeyAttributes(type);
            ...
            foreach (var kv in d)
            {
                ...
                if (jsonKeyAtts.FirstOrDefault(x => x?.JsonKey == n) is JsonKeyAttribute jsonKeyAttribute)
                {
                    name = jsonKeyAttribute.m_VariableName;
                }
                if (name == "$map")
                {
